...
// ✅ FIX: Imports para sincronização de créditos com timecode
import { useTimecodeSync } from "@/hooks/useTimecodeSync";
import { parseEstruturaPorCreditos, exibirDebugParser } from "@/lib/CreditoParser";
import { CreditosOverlay } from "@/components/CreditosOverlay";
import type { CreditoTimecode } from "@/lib/TimecodeSync";

export const Route = createFileRoute("/OK playout")({
...
<button onClick={async () => {
  if (!pgmARef.current) return;
  
  const analyzer = new AudioAnalyzer();
  await analyzer.inicializar(pgmARef.current);
  const segmentos = await analyzer.analisarAudio(pgmARef.current);
  const blocos = analyzer.agruparEmBlocos(segmentos);
  
  console.log("📊 Blocos detectados:", blocos);
  
  // Gerar lauda com timecodes
  const laudaNova = analyzer.gerarLaudaComTimecodes(blocos, materiaAtual?.estrutura || "");
  console.log("📝 Lauda atualizada:", laudaNova);
  
  toast.success(`${blocos.length} blocos detectados!`);
}}>
  🎙️ Analisar Áudio
</button>
...
interface ParseResult {
  ...
  itensLauda: ItemLauda[];
  ultimoOffTimecode: number; // ✅ FIX: Timecode do último OFF para créditos finais
  edTexto: string | null;
  edImagem: string | null;
}

function calcularDuracaoEstimada(texto: string | undefined): number {
  if (!texto) return 0;
  
  // ✅ FIX: Contagem mais precisa - palavras + espaços
  const palavras = texto.trim().split(/\s+/).length;
  const espacos = (texto.match(/\s/g) || []).length;
  
  // Cada palavra ~0.4s, cada espaço ~0.05s (pausa natural)
  // Média: ~2.5 palavras por segundo (mantém compatibilidade)
  const duracao = (palavras * 0.4) + (espacos * 0.05);
...
function parsarSonorasEPassagens(estrutura: string | null): ParseResult {
  ...
  let edTexto: string | null = null; // ✅ FIX: Rastreia editor de texto
  let edImagem: string | null = null; // ✅ FIX: Rastreia editor de imagens
  let ordem = 0;
  let ultimoOffTimecode = 0; // ✅ FIX: Rastreia timecode do último OFF para créditos finais

  if (!estrutura) {
    console.log("⚠️ Estrutura vazia");
    return { sonoras, passagens, producao, itensLauda, ultimoOffTimecode, edTexto, edImagem };
  }
  ...
  let i = 0;
  let tempoAcumulado = 0; // ✅ FIX: Rastreia tempo de todos os blocos anteriores (OFF + PASSAGEM + SONORA)

  while (i < linhas.length) {
    const linha = linhas[i];

    // ════ [OFF] - Captura narração em blocos abertos ════
    const offMatch = linha.match(/^\[OFF\s*\d*\]$/i);
    if (offMatch) {
      console.log("📢 Encontrado [OFF]");
      let textoOff = "";
      let timecodeFixo: number | null = null;
      
      // ✅ FIX: Verifica se há timecode fixo na linha anterior [MM:SS]
      if (i > 0) {
        const linhaAnterior = linhas[i - 1];
        const timecodeMatch = linhaAnterior.match(/^\[(\d{1,2}):(\d{2})\]\s*$/);
        if (timecodeMatch) {
          const minutos = parseInt(timecodeMatch[1], 10);
          const segundos = parseInt(timecodeMatch[2], 10);
          timecodeFixo = minutos * 60 + segundos;
          console.log(`⏱️ Timecode fixo OFF: ${timecodeFixo}s`);
        }
      }
      
      ultimoOffTimecode = timecodeFixo !== null ? timecodeFixo : tempoAcumulado; // ✅ FIX: Guarda timecode deste OFF (fixo ou calculado)

      // Coleta todas as linhas até o próximo tag
      while (i + 1 < linhas.length) {
        const proximaLinha = linhas[i + 1];
        if (proximaLinha.match(/^\[([A-Z_ÇÃÕ.]+)\]/i)) {
          break;
        }
        textoOff += (textoOff ? " " : "") + proximaLinha;
        i++;
      }

      if (textoOff) {
        const duracaoOff = calcularDuracaoEstimada(textoOff);
        
        // Só acumula tempo se NÃO foi dado timecode fixo
        if (timecodeFixo === null) {
          tempoAcumulado += duracaoOff;
        }
        
        console.log("✅ OFF capturado:", { duracao: duracaoOff + "s", timecodeFixo: timecodeFixo ? "SIM" : "NÃO", acumulado: tempoAcumulado + "s", ultimoOffTimecode });
      }
    }

    const sonoraMatch = linha.match(/^\[SONORA\]\s*(.*)$/i);
    if (sonoraMatch) {
      ...
      if (i > 0) {
        ...
        if (timecodeMatch) {
          ...
          const segundos = parseInt(timecodeMatch[2], 10);
          timecodeFixo = minutos * 60 + segundos;
          console.log(`⏱️ Timecode fixo SONORA: ${timecodeFixo}s (${minutos}:${timecodeMatch[2]})`);
        }
      }

      const inline = sonoraMatch[1].trim();
      if (inline) {
        // Formato novo (gerado pela redação): [SONORA] NOME - FUNÇÃO
        const partes = inline.split(/\s*-\s*/);
        nome = (partes[0] || "").trim();
        funcao = partes.slice(1).join(" - ").trim();
      } else if (i + 1 < linhas.length) {
        // Formato antigo: [SONORA] \n (NOME) \n (FUNÇÃO)
        const proximaLinha = linhas[i + 1];
      ...
      if (nome) {
        ...
        // ✅ FIX: Usa timecode fixo se fornecido, senão usa tempoAcumulado
        const timecodeInicio = timecodeFixo !== null ? timecodeFixo : tempoAcumulado;
        
        // Só acumula tempo se NÃO foi dado timecode fixo
        if (timecodeFixo === null) {
          tempoAcumulado += duracaoEstimada;
        }
        ...
        sonoras.push({ 
          ...
          texto, 
          duracaoEstimada,
          timecodeInicio // ← AGORA SINCRONIZADO COM OFF + PASSAGEM OU TIMECODE FIXO
        });
        itensLauda.push({
          tipo: "SONORA",
          nome: "SONORA",
        ...
        console.log("✅ Sonora:", { nome, funcao, duracao: duracaoEstimada + "s", timecodeInicio: timecodeInicio + "s", timecodeFixo: timecodeFixo ? "SIM" : "NÃO", acumulado: tempoAcumulado + "s" });
      }
    }

    // ════ [PASSAGEM] ════
    const passagemMatch = linha.match(/^\[PASSAGEM\]\s*(.*)$/i);
    if (passagemMatch) {
      ...
      const inline = passagemMatch[1].trim().replace(/:\s*$/, "");
      if (inline) {
        // Formato novo (gerado pela redação): [PASSAGEM] REPÓRTER // LOCAL [TIMECODE]
        // Ex: [PASSAGEM] SIDNEY PEREIRA // SÃO LUÍS MA [0:57]
        const partes = inline.split(/\s*\/\/\s*/);
        nome = (partes[0] || "").trim();
        local = partes.slice(1).join(" // ").trim();
        
        // ✅ FIX: Extrai timecode fixo se presente [MM:SS] ou [M:SS]
        const timecodeMatch = local.match(/\[(\d{1,2}):(\d{2})\]\s*$/);
        if (timecodeMatch) {
          const minutos = parseInt(timecodeMatch[1], 10);
          const segundos = parseInt(timecodeMatch[2], 10);
          timecodeFixo = minutos * 60 + segundos;
          // Remove o timecode do local
          local = local.replace(/\s*\[\d{1,2}:\d{2}\]\s*$/, "").trim();
          console.log(`⏱️ Timecode fixo encontrado: ${timecodeFixo}s (${minutos}:${timecodeMatch[2]})`);
        }
      } else if (i + 1 < linhas.length) {
        // Formato antigo: [PASSAGEM] \n (NOME) \n (LOCAL)
        const proximaLinha = linhas[i + 1];
        const nomeMatch = proximaLinha.match(/^\(([^)]+)\)$/);
        if (nomeMatch) {
          nome = nomeMatch[1].trim();
      ...
      // ✅ FIX: Captura texto da passagem (entre aspas ou até próximo tag)
      while (i + 1 < linhas.length) {
        const proximaLinha = linhas[i + 1];
        if (proximaLinha.match(/^\[([A-Z_ÇÃÕ.]+)\]/i)) {
          break;
      ...
      if (nome) {
        // ✅ FIX: Usa timecodeFixo se fornecido, senão usa tempoAcumulado
        const duracaoPassagem = calcularDuracaoEstimada(textoPassagem);
        const timecodePassagem = timecodeFixo !== null ? timecodeFixo : tempoAcumulado;
        if (duracaoPassagem > 0 && timecodeFixo === null) {
          tempoAcumulado += duracaoPassagem; // Só acumula se não houver timecode fixo
        }
        passagens.push({ nome, local, texto: textoPassagem, timecodeInicio: timecodePassagem });
        itensLauda.push({
          tipo: "PASSAGEM",
          nome: "PASSAGEM",
          valor: `${nome} - ${local}`,
        ...
        console.log("✅ Passagem adicionada:", { nome, local, duracao: duracaoPassagem + "s", timecodeInicio: timecodePassagem + "s", timecodeFixo: timecodeFixo !== null ? "SIM" : "NÃO", acumulado: tempoAcumulado + "s" });
      }
    }
    ...
    if (linha.match(/^\[ED[\._\s]*TEXTO\]$/i)) {
      ...
      if (i + 1 < linhas.length) {
        ...
        const nomeMatch = proximaLinha.match(/^\(([^)]+)\)$/);
        if (nomeMatch) {
          edTexto = nomeMatch[1].trim(); // ✅ FIX: Guarda em edTexto
          i++;
    ...
    if (linha.match(/^\[ED[\._\s]*IMAGENS?\]$/i)) {
      ...
      if (i + 1 < linhas.length) {
        ...
        const nomeMatch = proximaLinha.match(/^\(([^)]+)\)$/);
        if (nomeMatch) {
          edImagem = nomeMatch[1].trim(); // ✅ FIX: Guarda em edImagem
          i++;
  ...
  console.log("📊 Resultado final:", { sonoras, passagens, producao, itensLauda, ultimoOffTimecode, edTexto, edImagem });
...
function PlayoutPage() {
  ...
  const passagensMapRef = useRef<Map<string, { inicio: number; duracao: number }>>(new Map());
  const passagensDisparadosRef = useRef<Set<string>>(new Set());
  const gcAutoTimersRef = useRef<number[]>([]);
  ...
  const [passagensTimeline, setPassagensTimeline] = useState<Array<{
    nome: string;
    local: string;
    timecodeInicio: number;
    duracao: number;
  ...
  const [showTarjaCustomModal, setShowTarjaCustomModal] = useState(false);
  const [tarjaCustomPanelOpen, setTarjaCustomPanelOpen] = useState(false);
  const [tarjaCustomPanelPos, setTarjaCustomPanelPos] = useState({ x: 100, y: 100 });
  const tarjaCustomDragRef = useRef<{ startX: number; startY: number; startPX: number | string; startPY: number | string } | null>(null);
  ...
  const [syncSubtitleCredits, setSyncSubtitleCredits] = useState(true);  // ← Novo: sincronização ligada/desligada

  // ✅ FIX: State para créditos sincronizados com timecode do vídeo
  const [creditosComTimecode, setCreditosComTimecode] = useState<CreditoTimecode[]>([]);
  
  // ✅ FIX: Ref do player A precisa existir ANTES do hook useTimecodeSync usá-la
  const pgmARef = useRef<HTMLVideoElement>(null);

  // ✅ FIX: Hook para sincronização de créditos
  const { creditosAtivos, timecodeAtual, setCreditos, formatarTimecode } =
    useTimecodeSync(pgmARef);
  ...
  const [vuLevelL, setVuLevelL] = useState(0); // 0–100
  const [vuLevelR, setVuLevelR] = useState(0); // 0–100
  ...
  const extractSubtitlesFromMateria = (estrutura: string | null): string[] => {
    if (!estrutura) return [];
    
    const lines = estrutura.split('\n');
    const subtitles: string[] = [];
    const TEMPO_POR_LINHA = 3.5; // segundos - tempo médio de leitura
    let lineCounter = 0;
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith('[PASSAGEM]')) {
        let passagem = '';
        let linhasPassagem = 0;
        i++;
        lineCounter++;
        
        // Pega tudo até a próxima tag
        while (i < lines.length) {
          const nextLine = lines[i].trim();
          if (nextLine.startsWith('[') || nextLine === '') break;
          if (passagem.length > 0) passagem += ' ';
          passagem += nextLine.replace(/^["']|["']$/g, '');
          linhasPassagem++;
          i++;
          lineCounter++;
        }
        
        if (passagem.length > 15) {
          // Calcula timecode aproximado baseado em linhas
          const timecodeAprox = Math.round(lineCounter * TEMPO_POR_LINHA * 10) / 10;
          subtitles.push(passagem);
        }
        continue;
      }
      
      // ── SONORAS [SONORA] ──
      if (line.startsWith('[SONORA]')) {
        let nomeFunc = '';
        let timecodeFixo = '';
        i++;
        lineCounter++;
        ...
        while (i < lines.length) {
          ...
          // Verifica se tem timecode [MM:SS]
          const timeMatch = nextLine.match(/\[(\d{1,2}):(\d{2})\]/);
          if (timeMatch) {
            timecodeFixo = `${timeMatch[1]}:${timeMatch[2]}`;
          }
          
          if (nextLine.includes('-')) {
            nomeFunc = nextLine
              .replace(/^["']|["']$/g, '')
              .replace(/\[[\d:]+\]/g, '') // Remove timecode se houver
              .trim();
          }
          i++;
          lineCounter++;
      ...
      if (line.startsWith('[OFF')) {
        ...
        while (i < lines.length) {
          ...
          // Extrai timecode fixo se houver [MM:SS]
          const timeMatch = nextLine.match(/\[(\d{1,2}):(\d{2})\]/);
          if (timeMatch) {
            timecodeFixo = `${timeMatch[1]}:${timeMatch[2]}`;
          }
          ...
          off += nextLine
            .replace(/^["']|["']/g, '')
            .replace(/\[[\d:]+\]/g, '') // Remove timecode
            .replace(/\/\//g, ''); // Remove barras
          i++;
          lineCounter++;
        }
        
        // Pega primeira frase significativa do OFF
        const frases = off.split(/[.!?]/).filter(f => f.trim().length > 10);
        if (frases.length > 0) {
          subtitles.push(frases[0].trim());
        }
        continue;
      }
      
      // ── IMAGENS [IMAGENS] ──
      if (line.startsWith('[IMAGENS]') || line.startsWith('[IMAGEM]')) {
        let imagem = '';
        i++;
        lineCounter++;
        ...
        while (i < lines.length) {
          ...
          if (imagem.length > 0) imagem += ' ';
          imagem += nextLine.replace(/^["']|["']$/g, '');
          i++;
          lineCounter++;
        }
        
        if (imagem.length > 5) {
          subtitles.push(imagem);
        }
        continue;
      }
    ...
    // Filtra: remove vazio, muito curto, ou com muitos //
    return subtitles
  ...
  const progressRafRef = useRef<number | null>(null);
    ...
    materiaAtual.sonoras.forEach((sonora) => {
      const duracao = sonora.duracaoEstimada || 0;
      const inicio = sonora.timecodeInicio || 0; // ✅ FIX: Usa timecodeInicio do parser (já sincronizado com OFF + PASSAGEM)
      const key = `${sonora.nome}|${sonora.funcao}`;
      map.set(key, { inicio, duracao });
      timeline.push({
        nome: sonora.nome,
        funcao: sonora.funcao,
        timecodeInicio: inicio,
        duracao
      });
    });
  ...
  }, [materiaAtual?.sonoras]);

  // ✅ FIX: Efeito similar para PASSAGENS
  useEffect(() => {
    if (!materiaAtual?.passagens) {
      passagensMapRef.current.clear();
      passagensDisparadosRef.current.clear();
      setPassagensTimeline([]);
      return;
    }

    const map = new Map<string, { inicio: number; duracao: number }>();
    const timeline: typeof passagensTimeline = [];

    materiaAtual.passagens.forEach((passagem) => {
      const duracao = calcularDuracaoEstimada(passagem.texto || "");
      const inicio = passagem.timecodeInicio || 0;
      const key = `${passagem.nome}|${passagem.local}`;
      map.set(key, { inicio, duracao });
      timeline.push({
        nome: passagem.nome,
        local: passagem.local,
        timecodeInicio: inicio,
        duracao
      });
    });

    passagensMapRef.current = map;
    passagensDisparadosRef.current.clear();
    setPassagensTimeline(timeline);
  }, [materiaAtual?.passagens]);

  // ✅ FIX: Parsear estrutura e sincronizar créditos com timecode do vídeo
  useEffect(() => {
    if (!materiaAtual?.itensLauda && !materiaAtual?.estrutura) {
      setCreditosComTimecode([]);
      setCreditos([]);
      return;
    }

    // Parse da estrutura para extrair créditos com timecodes automáticos
    const { creditos, debug } = parseEstruturaPorCreditos(
      materiaAtual?.estrutura || ""
    );

    // ✅ FIX: Exibir debug no console para verificar parsing
    if (process.env.NODE_ENV === "development") {
      exibirDebugParser(debug);
    }

    // Atualiza state
    setCreditosComTimecode(creditos);

    // ✅ FIX: Sincroniza créditos com o vídeo (vai atualizar creditosAtivos)
    setCreditos(creditos);

    console.log(
      `🎬 ${creditos.length} créditos sincronizados com o vídeo`,
      creditos
    );
  }, [materiaAtual?.estrutura, materiaAtual?.itensLauda, setCreditos]);
   ...
   useEffect(() => {
    ...
    sonorasMapRef.current.forEach((timing, sonoraKey) => {
      ...
      // FIX: usa limiar (>=) em vez de janela estreita (±0.3s).
      // O evento timeupdate não dispara em intervalos perfeitamente regulares,
      // então uma janela estreita podia "passar batido" e nunca disparar o crédito.
      if (pgmCurrentTime >= timing.inicio) {
        const [nome, funcao] = sonoraKey.split('|');
        console.log(`🎤 AUTO-CRÉDITO SONORA: ${nome} (${funcao}) | ${pgmCurrentTime.toFixed(2)}s`);
        sonorasDisparadosRef.current.add(sonoraKey);
        handleGcTake(nome.trim(), funcao.trim(), timing.duracao);
      }
    });
  }, [pgmCurrentTime, isPlaying, sonorasTimeline]);

  // ✅ FIX: Efeito para disparar créditos de PASSAGENS
  useEffect(() => {
    if (!isPlaying || passagensTimeline.length === 0) return;

    passagensMapRef.current.forEach((timing, passagemKey) => {
      if (passagensDisparadosRef.current.has(passagemKey)) return;

      if (pgmCurrentTime >= timing.inicio) {
        const [nome, local] = passagemKey.split('|');
        console.log(`🎥 AUTO-CRÉDITO PASSAGEM: ${nome} (${local}) | ${pgmCurrentTime.toFixed(2)}s`);
        passagensDisparadosRef.current.add(passagemKey);
        handleGcTake(nome.trim(), local.trim(), timing.duracao);
      }
    });
  }, [pgmCurrentTime, isPlaying, passagensTimeline]);

  // ── FIX: insere automaticamente a fila de créditos (repórter/editor) ──
  // Antes, essa fila só era carregada (gcCreditsQueue) mas nunca exibida
  // sem clique manual em "GC TAKE". Agora dispara em sequência sozinha.
  const GC_AUTO_DELAY_INICIAL_MS = 2000; // entra 2s depois do VT começar
  const GC_AUTO_DURACAO_MS = 4000; // cada crédito fica 4s antes de avançar

  const scheduleAutoCredits = useCallback((creditsList: { line1: string; line2: string }[]) => {
    gcAutoTimersRef.current.forEach((id) => window.clearTimeout(id));
    gcAutoTimersRef.current = [];

    if (creditsList.length === 0) return;

    creditsList.forEach((credito, idx) => {
      const t = window.setTimeout(() => {
        setGcLine1(credito.line1);
        setGcLine2(credito.line2);
        setGcVisible(true);
        setGcCreditsQueue(creditsList.slice(idx + 1));
        if (pgmChannelRef.current?.readyState === 1) {
          pgmChannelRef.current.send(JSON.stringify({ type: "gc_show", line1: credito.line1, line2: credito.line2 }));
        }
      }, GC_AUTO_DELAY_INICIAL_MS + idx * GC_AUTO_DURACAO_MS);
      gcAutoTimersRef.current.push(t);
    });
    ...
    const tHide = window.setTimeout(() => {
      ...
      setGcLine2("");
      if (pgmChannelRef.current?.readyState === 1) {
        pgmChannelRef.current.send(JSON.stringify({ type: "gc_hide" }));
      }
    }, GC_AUTO_DELAY_INICIAL_MS + creditsList.length * GC_AUTO_DURACAO_MS);
    gcAutoTimersRef.current.push(tHide);
  }, []);

  // ✅ FIX: Efeito para disparar créditos FINAIS (PRODUÇÃO, ED. TEXTO, ED. IMAGEM) no último OFF
  const creditosFinaisDisparadosRef = useRef(false);
  useEffect(() => {
    if (!isPlaying || !materiaAtual) {
      creditosFinaisDisparadosRef.current = false;
      return;
    }

    // Se chegou ao timecode do último OFF e ainda não disparou créditos finais
    if (pgmCurrentTime >= materiaAtual.ultimoOffTimecode && !creditosFinaisDisparadosRef.current) {
      creditosFinaisDisparadosRef.current = true;
      
      const creditosFinais: { line1: string; line2: string }[] = [];
      
      if (materiaAtual.producao) {
        creditosFinais.push({ line1: materiaAtual.producao, line2: "PRODUÇÃO" });
      }
      if (materiaAtual.edTexto) {
        creditosFinais.push({ line1: materiaAtual.edTexto, line2: "EDITOR DE TEXTO" });
      }
      if (materiaAtual.edImagem) {
        creditosFinais.push({ line1: materiaAtual.edImagem, line2: "EDITOR DE IMAGENS" });
      }
      
      if (creditosFinais.length > 0) {
        console.log(`✅ DISPARANDO CRÉDITOS FINAIS em ${pgmCurrentTime.toFixed(2)}s:`, creditosFinais);
        scheduleAutoCredits(creditosFinais);
      }
    }
  }, [pgmCurrentTime, isPlaying, materiaAtual, scheduleAutoCredits]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // já conectado anteriormente — segue usando os analysers existentes
    }
  ...
  useEffect(() => {
    const ch = supabase
      .channel("desknews_playout_sync")
      .on("broadcast", { event: "RODA_VT" }, async ({ payload }) => {
        ...
        if (url && file) {
          ...
          if (aEl) {
            ...
            aEl.load();
            aEl.addEventListener("canplay", () => { aEl.play().catch(() => {}); }, { once: true });
          }
        ...
        if (materiaId) {
          try {
            ...
            if (!error && data) {
              ...
              // FIX: antes a fila só era carregada, nunca inserida no ar
              // automaticamente. Agora dispara a sequência sozinha.
              scheduleAutoCredits(creditsList);
              ...
              const novasLegendas = extractSubtitlesFromMateria(data.estrutura);
              setSubtitleQueue(novasLegendas);
              setCurrentSubtitleIndex(0);
              if (novasLegendas.length > 0) setSubtitleText(novasLegendas[0]);
  ...
  useEffect(() => {
    if (!subtitleEnabled || subtitleQueue.length === 0) return;
    const interval = setInterval(() => {
      setCurrentSubtitleIndex(prev => {
        const next = (prev + 1) % subtitleQueue.length;
        setSubtitleText(subtitleQueue[next]);
        
        // ── SINCRONIZAÇÃO DE CRÉDITOS COM LEGENDAS ──
        // Só sincroniza se o flag estiver ligado
        if (syncSubtitleCredits && materiaAtual && materiaAtual.sonoras && materiaAtual.sonoras.length > 0) {
          const sonoraAtual = materiaAtual.sonoras[next % materiaAtual.sonoras.length];
          if (sonoraAtual) {
            setGcLine1(sonoraAtual.nome.toUpperCase());
            setGcLine2(sonoraAtual.funcao.toUpperCase());
            setGcVisible(true);
            
            if (pgmChannelRef.current?.readyState === 1) {
              pgmChannelRef.current.send(JSON.stringify({
                type: "gc_show_sync",
                line1: sonoraAtual.nome.toUpperCase(),
                line2: sonoraAtual.funcao.toUpperCase(),
              }));
            }
          }
        }
    ...
    return () => clearInterval(interval);
  }, [subtitleEnabled, subtitleQueue, materiaAtual, syncSubtitleCredits]);
  ...
  const handleTake = () => {
    ...
    // Encerra vídeo e áudio de qualquer CAM (iframe) anterior antes de remover
    if (pgmCamUrl) sendPgmCamCommand("stopVideo");
    setPgmCamUrl(null); // limpa qualquer stream de cam anterior
    setPgmCamIsYoutube(false);
    setPgmCamMuted(false);
    setPgmCamVolume(100);
  ...
  (nome: string, funcao: string, autoDuration?: number) => {
    ...
    // FIX: este disparo automático não notificava o canal da TV (apenas a
    // preview local era atualizada). Sem isso, o crédito "aparecia" no
    // operador mas nunca ia ao ar de fato.
    if (pgmChannelRef.current?.readyState === 1) {
      pgmChannelRef.current.send(JSON.stringify({ type: "gc_show", line1: nome.toUpperCase(), line2: funcao.toUpperCase() }));
    }
  ...
  const handleGcTakeQueue = () => {
    // Intervenção manual cancela a sequência automática pendente
    gcAutoTimersRef.current.forEach((id) => window.clearTimeout(id));
    gcAutoTimersRef.current = [];
  ...
  return (
    <div className="fixed inset-0 text-white flex flex-col overflow-hidden font-sans" style={{ backgroundColor: '#121212', color: 'white' }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto p-3 gap-3" style={{ backgroundColor: '#121212' }}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {/* PREVIEW */}
            <div className="flex flex-col gap-1.5 rounded-xl border p-2" style={{ backgroundColor: '#161616', borderColor: '#00E67625' }}>
              ...
              <div className="relative aspect-video bg-black rounded-lg overflow-hidden border" style={{ borderColor: isCamPreview ? '#f97316' : '#00E67630' }}>
                ...
                >
                  <track kind="captions" src="" style={{ display: "none" }} />
                </video>
            ...
            {/* PROGRAM */}
            <div className="flex flex-col gap-1.5 rounded-xl border p-2" style={{ backgroundColor: '#161616', borderColor: '#ef444425' }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
              ...
              >
                ...
                {pgmFile && items[currentIndex - 1]?.cabeca && (
                  <div className="absolute bottom-4 left-4 right-4 z-30">
                    <div className="bg-black/90 border-l-4 border-red-600 px-3 py-1.5">
                      <div className="text-[10px] font-bold text-white">{items[currentIndex - 1]?.cabeca}</div>
                ...
                {subtitleEnabled && subtitleText && (
                  <div className="absolute bottom-12 left-0 right-0 z-35 px-3">
                    <div className="bg-black/95 border-t-2 border-blue-500 px-4 py-2.5 rounded-sm">
                      <div className="text-white text-[11px] font-medium leading-tight text-center max-w-[90%] mx-auto line-clamp-2">
                        {subtitleText}
                      </div>
                    </div>
                  </div>
                ...
                <button
                  ...
                  className={cn(
                    "absolute top-2 right-2 flex items-center gap-1.5 px-2 py-1 rounded-full text-[8px] font-bold uppercase tracking-widest transition-all z-20",
                    subtitleEnabled
                      ? "bg-blue-600 text-white hover:bg-blue-700"
                      : "bg-zinc-800 text-zinc-500 hover:bg-zinc-700"
                  )}
                  title={subtitleEnabled ? "Desativar legendas" : "Ativar legendas"}
                >
                  <Subtitles className="h-3 w-3" />
                  CC
                </button>

                {/* TOGGLE SYNC CREDITS BUTTON */}
                <button
                  onClick={() => setSyncSubtitleCredits(!syncSubtitleCredits)}
                  className={cn(
                    "absolute top-2 right-20 flex items-center gap-1.5 px-2 py-1 rounded-full text-[8px] font-bold uppercase tracking-widest transition-all z-20",
                    syncSubtitleCredits
                      ? "bg-purple-600 text-white hover:bg-purple-700"
                      : "bg-zinc-800 text-zinc-500 hover:bg-zinc-700"
                  )}
                  title={syncSubtitleCredits ? "Desativar sincronização de créditos" : "Ativar sincronização de créditos"}
                >
                  <span>♪</span>
                  SYNC
                </button>
                ...
                {/* ✅ FIX: CRÉDITOS SINCRONIZADOS COM TIMECODE DO VÍDEO */}
                {syncSubtitleCredits && creditosAtivos.length > 0 && (
                  <div className="absolute inset-0 w-full h-full pointer-events-none z-40">
                    <CreditosOverlay
                      creditosAtivos={creditosAtivos}
                      timecodeAtual={timecodeAtual}
                      posicao="baixo"
                      tamanho="medio"
                      autoFade={true}
                    />
              ...
              {/* VU METER ESTÉREO (L/R) — fora do monitor, alinhado com a busca do YouTube */}
              <div className="flex flex-col gap-1 mt-1.5">
                {[{ label: "L", level: vuLevelL }, { label: "R", level: vuLevelR }].map(({ label, level }) => (
                  <div key={label} className="flex items-center gap-1.5">
                    <span className="text-[8px] font-mono font-bold text-zinc-600 w-2.5">{label}</span>
                    <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: '#0f0f0f', border: '1px solid #2a2a2a' }}>
                      <div className="flex gap-[2px] h-full items-center px-0.5">
            ...
            <div className="flex flex-col gap-1.5 rounded-xl border p-2" style={{ backgroundColor: '#161616', borderColor: multiviewActive ? '#27272a' : '#ef444425' }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <Youtube className={cn("h-3 w-3", multiviewActive ? "text-zinc-400" : "text-red-500")} />
                  <span className={cn("text-[10px] font-bold uppercase tracking-widest", multiviewActive ? "text-zinc-300" : "text-red-500")}>{multiviewActive ? "MONITOR (MULTIVIEW PRA 4 CAMERAS)" : "YouTube"}</span>
                </div>
                {youtubeVisible && (
                  <button
          ...
          <div className="grid grid-cols-[auto_1fr_auto_1fr_auto_auto] gap-0 flex-1 min-h-0">
            ...
            <div className="border rounded-xl p-4 flex flex-col gap-3 w-[380px]" style={{ backgroundColor: '#1a1a1a', borderColor: '#2a2a2a' }}>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 h-1.5 bg-zinc-800 rounded-full cursor-pointer relative overflow-hidden group" onClick={handleSeek}>
                  <div className="h-full rounded-full transition-none relative" style={{ width: `${pgmProgress}%`, backgroundColor: '#ef4444' }}>
            ...
            {/* Botão Tarja Personalizada */}
            <button onClick={() => setTarjaCustomPanelOpen(!tarjaCustomPanelOpen)}
              className="w-full px-2 py-2 rounded-lg border text-[9px] font-bold uppercase tracking-widest transition-all active:scale-[0.98]"
              style={{ backgroundColor: tarjaCustomPanelOpen ? '#8b5cf640' : '#8b5cf620', borderColor: '#8b5cf640', color: '#a78bfa' }}>
              🎞 Tarja Personalizada
            </button>
          ...
          {/* ── TELA DE GC COMPLETA ── */}
          <div className="border rounded-xl p-3 flex flex-col gap-3" style={{ backgroundColor: '#161b16', borderColor: '#00E67650' }}>
            <span className="text-[11px] font-black uppercase tracking-widest" style={{ color: '#00E676' }}>Tela de GC Completa</span>
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex flex-col gap-0.5">
                  ...
                  <input type="text" value={gcLine1} onChange={(e) => setGcLine1(e.target.value)}
                    placeholder="Nome / Título"
                    className="w-full border rounded px-2 py-1 text-[10px] font-mono text-zinc-200 placeholder:text-zinc-700 focus:outline-none transition-all"
                    style={{ backgroundColor: '#0f140f', borderColor: '#2a2a2a' }}
                  />
                </div>
                <div className="flex flex-col gap-0.5">
                  <label className="text-[8px] uppercase tracking-widest text-zinc-600">Linha 2 — Cargo</label>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <select value={tarjaFont} onChange={(e) => setTarjaFont(e.target.value)}
                    className="border rounded px-1.5 py-1 text-[9px] font-mono text-zinc-300 focus:outline-none"
                    style={{ backgroundColor: '#0f140f', borderColor: '#2a2a2a' }}>
                    <option value="sans-serif">Tipo de fonte: Sans</option>
                    <option value="serif">Tipo de fonte: Serif</option>
                    <option value="monospace">Tipo de fonte: Mono</option>
                  </select>
                  <select
                    className="border rounded px-1.5 py-1 text-[9px] font-mono text-zinc-500 focus:outline-none"
                    style={{ backgroundColor: '#0f140f', borderColor: '#2a2a2a' }}>
                    <option>Tipo de fonte (linha 2)</option>
                  </select>
                </div>
              </div>
              <div className="relative w-full aspect-video bg-black rounded border overflow-hidden" style={{ borderColor: '#2a2a2a' }}>
                <div className="absolute inset-0 flex items-center justify-center text-zinc-800 text-[6px] uppercase tracking-widest">Tela GC Completa</div>
                {(gcLine1 || gcLine2) ? (
                  <div className="absolute bottom-0 left-0 right-0 px-0.5 pb-0.5">
                    <div className="flex items-stretch overflow-hidden rounded-sm">
                      <div className="w-0.5 bg-emerald-500 shrink-0" />
                      <div className="bg-black/90 px-1 py-0.5 flex-1 min-w-0">
                        {gcLine1 && <div className="text-white font-bold text-[6px] uppercase truncate">{gcLine1}</div>}
                        {gcLine2 && <div className="text-zinc-400 text-[5px] uppercase truncate">{gcLine2}</div>}
                      </div>
            ...
            {/* tamanho da tarja | personalizar tarja */}
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <label className="text-[8px] uppercase tracking-widest text-zinc-500">Tamanho da tarja</label>
                  <span className="text-[9px] font-mono text-zinc-400">{tarjaScaleX}</span>
                </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <label className="text-[8px] uppercase tracking-widest text-zinc-500">Personalizar tarja</label>
                  <input type="color"
                    value={`#${Math.round(((tarjaHue/360))*255).toString(16).padStart(2,'0')}${Math.round((tarjaSaturation/100)*255).toString(16).padStart(2,'0')}cc`}
                    onChange={(e) => {
                      ...
                      setTarjaHue(h); setTarjaSaturation(s);
                    }}
                    className="w-5 h-5 rounded border-0 cursor-pointer bg-transparent"
                    title="Cor personalizada da tarja"
                  />
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <label className="text-[8px] uppercase tracking-widest text-zinc-500">Posição X</label>
                  <input type="number" value={tarjaX} onChange={(e) => setTarjaX(Number(e.target.value))}
                    className="w-12 text-right text-[9px] font-mono rounded border px-1 py-0.5 text-zinc-300"
                    style={{ backgroundColor: '#0f140f', borderColor: '#2a2a2a' }} />
                </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <label className="text-[8px] uppercase tracking-widest text-zinc-500">Posição Y</label>
                  <input type="number" value={tarjaY} onChange={(e) => setTarjaY(Number(e.target.value))}
                    className="w-12 text-right text-[9px] font-mono rounded border px-1 py-0.5 text-zinc-300"
                    style={{ backgroundColor: '#0f140f', borderColor: '#2a2a2a' }} />
                </div>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <label className="text-[8px] uppercase tracking-widest text-zinc-500">Escala</label>
                <span className="text-[9px] font-mono text-zinc-400">{(tarjaScaleX / 100).toFixed(2)}</span>
              </div>
              <input type="range" min={10} max={400} value={tarjaScaleX}
                onChange={(e) => { const v = Number(e.target.value); setTarjaScaleX(v); if (tarjaScaleLock) setTarjaScaleY(v); }}
                className="w-full h-1 accent-emerald-500" />
            </div>

            {/* FADE IN / FADE OUT / UPDATE LIVE */}
            <div className="grid grid-cols-3 gap-1.5">
              <button
                onClick={() => { setTarjaVisible(true); pgmChannelRef.current?.readyState === 1 && pgmChannelRef.current.send(JSON.stringify({ type: "tarja_show" })); }}
                className="px-2 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest border transition-all active:scale-[0.97]"
                style={{ backgroundColor: '#16a34a20', borderColor: '#16a34a50', color: '#4ade80' }}>
                Fade In
              </button>
              <button
                onClick={() => { setTarjaVisible(false); pgmChannelRef.current?.readyState === 1 && pgmChannelRef.current.send(JSON.stringify({ type: "tarja_hide" })); }}
                className="px-2 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest border transition-all active:scale-[0.97]"
                style={{ backgroundColor: '#dc262620', borderColor: '#dc262650', color: '#f87171' }}>
                Fade Out
              </button>
              <button
                onClick={() => { pgmChannelRef.current?.readyState === 1 && pgmChannelRef.current.send(JSON.stringify({ type: "tarja_update", text: tarjaText, hue: tarjaHue, saturation: tarjaSaturation, alpha: tarjaAlpha, x: tarjaX, y: tarjaY, scaleX: tarjaScaleX, scaleY: tarjaScaleY })); toast.success("Tarja atualizada ao vivo"); }}
                className="px-2 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest border text-zinc-300 transition-all active:scale-[0.97]"
                style={{ backgroundColor: '#252525', borderColor: '#3a3a3a' }}>
                Update Live
              </button>
            </div>

            {/* FADE / FOR / TEXT PRESET */}
            <div className="grid grid-cols-3 gap-1.5">
              <button disabled className="px-2 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest border text-zinc-600 cursor-not-allowed" style={{ backgroundColor: '#1c1c1c', borderColor: '#2a2a2a' }}>
                Fade
              </button>
              <button disabled className="px-2 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest border text-zinc-600 cursor-not-allowed" style={{ backgroundColor: '#1c1c1c', borderColor: '#2a2a2a' }}>
                For
              ...
              <button
                onClick={() => { setTarjaText(gcLine1 || tarjaText); toast.success("Preset de texto aplicado"); }}
                className="px-2 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest border text-zinc-300 transition-all active:scale-[0.97]"
                style={{ backgroundColor: '#252525', borderColor: '#3a3a3a' }}>
                Text Preset
              </button>
          ...
          <div className="border rounded-xl p-3 flex flex-col gap-2" style={{ backgroundColor: '#1a1a1a', borderColor: '#2a2a2a' }}>
            ...
            <div className="grid grid-cols-2 gap-3 p-3 bg-[#1a1a1a] rounded border border-[#2a2a2a]">
              {[0, 1, 2, 3].map((id) => (
                <button
                  key={id}
                  onClick={() => handleMeFrameClick(id)}
                  className={cn(
                    "aspect-square rounded-full border-2 flex flex-col items-center justify-center transition-all group overflow-hidden relative mx-auto w-20 h-20",
                    meActiveFrame === id ? "border-[#00E676] shadow-[0_0_14px_rgba(0,230,118,0.6)]" : "border-cyan-500/60 hover:border-[#00E676]"
                  )}
                ...
                >
                  {meFrames[id] ? (
                    <>
                      <img src={meFrames[id] as string} alt={`Frame ${id + 1}`} className="absolute inset-0 w-full h-full object-contain p-1 rounded-full" />
                      <button
                  ...
                  <span className={cn(
                    "text-[9px] mt-1 z-10 px-1 rounded font-black uppercase tracking-widest",
                    meActiveFrame === id ? "text-[#00E676] bg-black/60" : "text-cyan-300 bg-black/60"
                  )}>
                    FRAME {id + 1}
                  </span>
                  <input
            ...
            {/* ─ Alavanca de Corte de Imagem ─ */}
            <div className="border rounded-xl p-3 flex flex-col items-center justify-center gap-2 overflow-hidden" style={{ backgroundColor: '#1a1a1a', borderColor: '#2a2a2a' }}>
              <div className="flex items-center gap-3">
                <span className="text-[9px] uppercase tracking-widest text-zinc-500 font-bold writing-mode-vertical">condio bezel</span>
                ...
                <div
                  ...
                  style={{
                    ...
                    background: "linear-gradient(155deg, #2c2c2e, #0a0a0a 70%)",
                    clipPath: "polygon(18% 0%, 82% 0%, 100% 100%, 0% 100%)",
                    boxShadow: "inset 0 2px 4px rgba(255,255,255,0.06), inset 0 -4px 10px rgba(0,0,0,0.8)",
                  }}
                >
                  {/* parafusos */}
                  {[["8%","6%"],["92%","6%"],["2%","94%"],["98%","94%"]].map(([l,t],i)=>(
                    <div key={i} className="absolute rounded-full" style={{ left: l, top: t, transform: "translate(-50%,-50%)", width: 7, height: 7, background: "radial-gradient(circle at 35% 35%, #888, #1a1a1a)", boxShadow: "0 1px 1px rgba(0,0,0,0.8)" }} />
                  ))}

                  {/* régua esquerda */}
                  <div className="absolute left-[14%] top-[14%] bottom-[10%] flex flex-col justify-between">
                    {Array.from({ length: 10 }).map((_, i) => (
                      <div key={i} style={{ width: i % 3 === 0 ? 7 : 4, height: 1, background: (9 - i) <= Math.round((transValue / 100) * 9) ? "#f97316" : "#4a4a4a" }} />
                    ))}
                  </div>
                  {/* régua direita */}
                  <div className="absolute right-[14%] top-[14%] bottom-[10%] flex flex-col justify-between items-end">
                    {Array.from({ length: 10 }).map((_, i) => (
                      <div key={i} style={{ width: i % 3 === 0 ? 7 : 4, height: 1, background: (9 - i) <= Math.round((transValue / 100) * 9) ? "#f97316" : "#4a4a4a" }} />
                    ))}
                  </div>
                  ...
                  {/* fenda central */}
                  <div className="absolute left-1/2 top-[10%] bottom-[8%] -translate-x-1/2 rounded-full" style={{ width: 26, background: "linear-gradient(90deg,#000,#1c1c1c 40%,#000)", boxShadow: "inset 0 0 8px rgba(0,0,0,0.95)" }} />

                  {/* indicador triangular */}
                  <div
                    className="absolute left-1/2 pointer-events-none"
                    style={{
                      top: `calc(${100 - transValue}% * 0.78 + 12%)`,
                      transform: "translate(-50%,-50%)",
                      width: 0, height: 0,
                      borderLeft: "5px solid transparent",
                      borderRight: "5px solid transparent",
                      borderTop: "7px solid #f97316",
                      filter: transValue > 0 ? "drop-shadow(0 0 3px rgba(249,115,22,0.8))" : "none",
                    }}
                  />
                  ...
                  <div
                    ...
                    style={{
                      width: 30, cursor: "ns-resize",
                      top: `calc(${100 - transValue}% * 0.74 + 6%)`,
                      height: 60,
                      transform: "translate(-50%,-30%)",
                      background: "linear-gradient(180deg,#000 0%,#26262a 14%, #3a3a3e 100%)",
                    ...
                    onMouseDown={(e) => {
                      ...
                      const onMove = (ev: MouseEvent) => {
                        const pct = Math.min(100, Math.max(0, 100 - ((ev.clientY - rect.top) / rect.height) * 100));
                        setTransValue(Math.round(pct));
                        applyTransOpacity(Math.round(pct), activePlayer);
                      };
                      const onUp = (ev: MouseEvent) => {
                    ...
                    title="Arraste para fazer a transição"
                  >
                    {/* punho cromado cilíndrico no topo */}
                    <div
                      className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full"
                      style={{
                        width: 46, height: 22,
                        background: "linear-gradient(180deg,#fafafa 0%,#bdbdbd 35%,#6b6b6b 65%,#e8e8e8 100%)",
                        boxShadow: "0 2px 4px rgba(0,0,0,0.6), inset 0 1px 1px rgba(255,255,255,0.7)",
                ...
                {/* Botões CUT / DISSOLVE glossy 2x2 com seta */}
                <div className="relative flex items-center">
                  <div className="grid grid-cols-2 gap-2">
                    {(["CUT", "DISSOLVE", "CUT", "DISSOLVE"] as const).map((label, i) => (
                      <button
                        key={i}
                        onClick={() => {
                          if (label === "CUT") {
                            ...
                            const id = setInterval(() => {
                              ...
                              setTransValue(clamped);
                              applyTransOpacity(clamped, activePlayer);
                              if (clamped >= 100) { clearInterval(id); handleTransComplete(); }
                            }, 25);
                          }
                        }}
                        className="w-12 h-9 rounded-md text-[7px] font-black uppercase tracking-tighter transition-all active:scale-[0.93]"
                        style={{
                          background: "linear-gradient(180deg,#fdfdfd 0%,#e4e4e7 45%,#c7c7cc 55%,#fdfdfd 100%)",
                          border: "1px solid #999",
                          color: "#27272a",
                          boxShadow: "0 2px 3px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.9)",
                        }}
                      >
                        {label}
                      </button>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderRight: "9px solid #52525b",
                    }}
                  />
                </div>
              </div>
              <span className="text-[8px] uppercase tracking-widest text-zinc-600 font-bold text-center">alavanca de corte de imagem</span>
            </div>
      ...
      {/* PAINEL FLUTUANTE: Tarja Personalizada */}
      {tarjaCustomPanelOpen && (
        <div
          className="fixed z-[9999] border rounded-2xl shadow-2xl w-96 select-none"
          style={{ left: tarjaCustomPanelPos.x, top: tarjaCustomPanelPos.y, backgroundColor: '#1a1a1a', borderColor: '#3a3a3a' }}
        >
          {/* Header arrastável */}
          <div
            className="flex items-center justify-between px-4 py-3 rounded-t-2xl cursor-grab active:cursor-grabbing border-b"
            style={{ backgroundColor: '#252525', borderColor: '#333' }}
            onMouseDown={(e) => {
              const startX = typeof tarjaCustomPanelPos.x === 'string' ? parseInt(tarjaCustomPanelPos.x) : tarjaCustomPanelPos.x;
              const startY = typeof tarjaCustomPanelPos.y === 'string' ? parseInt(tarjaCustomPanelPos.y) : tarjaCustomPanelPos.y;
              tarjaCustomDragRef.current = { startX: e.clientX, startY: e.clientY, startPX: startX, startPY: startY };
              const onMove = (ev: MouseEvent) => {
                if (!tarjaCustomDragRef.current) return;
                setTarjaCustomPanelPos({ 
                  x: (tarjaCustomDragRef.current.startPX as number) + ev.clientX - tarjaCustomDragRef.current.startX, 
                  y: (tarjaCustomDragRef.current.startPY as number) + ev.clientY - tarjaCustomDragRef.current.startY 
                });
              };
              const onUp = () => {
                tarjaCustomDragRef.current = null;
                window.removeEventListener("mousemove", onMove);
                window.removeEventListener("mouseup", onUp);
              };
              window.addEventListener("mousemove", onMove);
          ...
          >
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-300">🎞 Tarja Personalizada</span>
            <button onClick={() => setTarjaCustomPanelOpen(false)} className="text-zinc-500 hover:text-red-400 font-bold transition-colors">✕</button>
          </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  ...
                  <button
                    onClick={() => {
                      setGcLine1("");
                      setGcLine2("");
                      pgmChannelRef.current?.readyState === 1 && pgmChannelRef.current.send(JSON.stringify({ type: "gc_clear" }));
                    }}
                    style={{
                      flex: 1,
                      padding: "8px",
