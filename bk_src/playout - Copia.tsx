...
function PlayoutPage() {
  ...
  const progressRafRef = useRef<number | null>(null);
    ...
    materiaAtual.sonoras.forEach((sonora) => {
      ...
      // Usa timecode manual se definido; senão usa acumulado automático
      const inicio = sonora.timecodeInicio !== undefined ? sonora.timecodeInicio : acumulado;
      ...
      console.log(`⏱️ ${sonora.nome} → ${inicio}s por ${duracao}s ${sonora.timecodeInicio !== undefined ? "(manual)" : "(automático)"}`);
      if (sonora.timecodeInicio === undefined) acumulado += duracao;
    });
   ...
   useEffect(() => {
    if (!isPlaying || pgmCurrentTime === 0 || sonorasTimeline.length === 0) return;

    sonorasMapRef.current.forEach((timing, sonoraKey) => {
      if (sonorasDisparadosRef.current.has(sonoraKey)) return;
      
      if (Math.abs(pgmCurrentTime - timing.inicio) < 0.3) {
        const [nome, funcao] = sonoraKey.split('|');
        console.log(`🎤 AUTO-CRÉDITO: ${nome} (${funcao}) | ${pgmCurrentTime.toFixed(2)}s`);
  ...
  const handleTake = () => {
    ...
    if (aEl) {
      // Listener ANTES do load — evita race condition quando arquivo está em cache
      aEl.addEventListener("canplay", () => { aEl.play().catch(() => {}); }, { once: true });
    ...
    setPgmProgress(0);
    setPgmCurrentTime(0);
    setIsPlaying(true); // Define como tocando
    if (pgmChannelRef.current?.readyState === 1)
