...
import React from "react"; // ✅ FIX: import no topo, não no final
...
class RBACManager {
  ...
  async initUser(): Promise<UserRole | null> {
    // ✅ FIX: Impede erro no servidor (SSR) onde localStorage não existe
    if (typeof window === "undefined") return null;

    // 1. Tentar obter o usuário salvo pelo nosso backend Express (armazenado no auth.js)
    ...
    if (storedUser && storedUser !== "undefined") {
      try {
        ...
        if (user && user.id) {
          this.userId = user.id;
          this.userRole = (user.role as UserRole) || "reporter";
    ...
    // 2. Fallback para o Supabase Auth caso o login tenha sido feito por lá
    try {
      const { data, error } = await supabase.auth.getUser();
      
      if (data?.user) {
        this.userId = data.user.id;
        this.userRole = (data.user.user_metadata?.role as UserRole) || "reporter";
        return this.userRole;
      }
      return null;
    } catch (error) {
  ...
  canEdit(resource: string, ownerId?: string): boolean {
    ...
    // ✅ FIX: uso do userId em cache (síncrono) em vez de chamar getUser() que é Promise
    if (ownerId) {
...
export function RequiresPermission({
  ...
  // ✅ FIX: aguardar carregamento antes de renderizar fallback
