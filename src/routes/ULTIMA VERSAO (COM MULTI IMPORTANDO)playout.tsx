...
export const Route = createFileRoute("/ULTIMA VERSAO (COM MULTI IMPORTANDO)playout")({
  ...
  const handleTake = () => {
    ...
    if (isCamPreview && previewCamIdx !== null) {
      ...
      if (camEl && camEl.src && aEl) {
        ...
        setActivePlayer("A");
        setIsPlaying(true);
        // cria um LocalVideoFile sintético para exibir no PGM
        const camFile: LocalVideoFile = {
          name: `CAM ${previewCamIdx + 1}`,
          sizeMB: 0,
          lastModified: "",
          codec: "H.264",
          blobUrl: src,
        ...
        setPgmFile(camFile);
        if (pgmChannelRef.current?.readyState === 1)
          pgmChannelRef.current.send(JSON.stringify({ type: "cam_pgm", cam: previewCamIdx + 1 }));
        toast.success(`CAM ${previewCamIdx + 1} → PROGRAM`);
      }
      return;
    }
              ...
              {multiviewActive ? (
                <div className="grid grid-cols-2 gap-1 aspect-video bg-black rounded-lg overflow-hidden border border-cyan-600/20">
                  {[1, 2, 3, 4].map((cam) => {
                    ...
                    // Embed URL para streams (YouTube, Instagram etc)
