...
export const Route = createFileRoute("/MULTI VIEW COM BOTOES - playout")({
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {multiviewActive ? (
                <div className="grid grid-cols-2 gap-1 aspect-video bg-black rounded-lg overflow-hidden border border-cyan-600/20">
                  {[1, 2, 3, 4].map((cam) => (
                    <div
                      key={cam}
                      className="relative bg-black flex items-center justify-center cursor-pointer transition-all duration-150"
                      style={{ border: activeCam === cam ? '2px solid #f97316' : '1px solid rgba(8,145,178,0.3)' }}
                      onClick={() => {
                        setActiveCam(cam);
                        // Envia sinal para PGM via WebSocket
                        if (pgmChannelRef.current?.readyState === 1) {
                          pgmChannelRef.current.send(JSON.stringify({ type: "cam_take", cam }));
                        }
                        toast.success(`CAM ${cam} → PGM`);
                      }}
                    >
                      {cam === 1 && youtubeVisible && youtubeUrl ? (
                        <iframe key={youtubeUrl} src={youtubeUrl}
                          className="absolute inset-0 w-full h-full"
                          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                          allowFullScreen title="YouTube Monitor" style={{ border: "none", pointerEvents: "none" }}
                        />
                      ) : (
                        <div className="flex items-center justify-center text-[10px] font-bold" style={{ color: activeCam === cam ? '#f97316' : '#3f3f46' }}>CAM {cam}</div>
                      )}
                      {activeCam === cam && (
                        <div className="absolute top-1 left-1 bg-orange-500 text-black text-[7px] font-black px-1 py-0.5 rounded uppercase tracking-widest z-10">PGM</div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <button
                  onClick={() => setMultiviewActive(!multiviewActive)}
                  ...
                  style={{ backgroundColor: multiviewActive ? '#0891b2' : '#0891b2' }}
                  title="4 Câmeras"
                >
