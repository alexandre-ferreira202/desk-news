...
export const Route = createFileRoute("/bk playout")({
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto p-5 gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                {gcVisible && (gcLine1 || gcLine2) && (
                  <div className="absolute bottom-0 left-0 right-0 z-40 px-3 pb-3 animate-in slide-in-from-bottom duration-300"> {/* Premium GC overlay */}
          ...
          <div className="bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-3xl p-6 flex flex-col gap-5 shadow-[var(--shadow-xl)]">
            {/* Header */}
            <div className="flex items-center gap-2">
              <Type className="h-3.5 w-3.5 text-[var(--text-tertiary)]" />
              <span className="text-label font-bold uppercase tracking-widest text-[var(--text-tertiary)]">GC — Gerador de Caracteres</span>
            </div>

            {/* Campos de entrada + Preview (2 colunas no topo) */}
            <div className="grid grid-cols-[3fr_1fr] gap-4"> {/* Premium GC inputs */}
              {/* Campos de entrada */}
              <div className="flex flex-col gap-2">
                <div className="flex flex-col gap-1"> {/* Premium input */}
                  <label className="text-[9px] uppercase tracking-widest text-zinc-600">Linha 1 — Nome / Título</label>
                  <input
                    type="text"
                    value={gcLine1}
                    onChange={(e) => setGcLine1(e.target.value)}
                    placeholder="Nome / Título"
                    className="w-full bg-zinc-800/60 border border-white/5 rounded-lg px-3 py-1.5 text-[11px] font-mono text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-red-500/40 transition-all"
                  />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] uppercase tracking-widest text-zinc-600">Linha 2 — Cargo / Informação</label>
                  <input
                    type="text"
                    value={gcLine2}
                ...
                {/* Duração (fade out automático) */}
                <div className="flex items-center gap-2">
                  <label className="text-[9px] uppercase tracking-widest text-zinc-600 w-20 shrink-0">Duração:</label>
                  <select
                    value={gcDuration}
                    onChange={(e) => setGcDuration(Number(e.target.value))}
                    className="flex-1 bg-zinc-800/60 border border-white/5 rounded-lg px-2 py-1.5 text-[10px] font-mono text-zinc-200 focus:outline-none focus:border-red-500/40 transition-all"
                  >
              ...
              {/* Preview Miniatura */}
              <div className="relative w-full aspect-video bg-zinc-950 rounded-lg overflow-hidden border border-white/5 shrink-0">
                <div className="absolute inset-0 flex items-center justify-center text-zinc-800 text-[7px] uppercase tracking-widest text-center px-1">
                  GC Live Prev
                </div>

                {gcLine1 || gcLine2 ? (
                  <div className="absolute bottom-0 left-0 right-0 px-1 pb-1">
                    <div className="flex items-stretch overflow-hidden rounded-xs">
                      <div className="w-0.5 bg-red-600 shrink-0" />
                      <div className="bg-black/90 px-1 py-0.5 flex-1 min-w-0">
                        {gcLine1 && (
                          <div className="text-white font-bold text-[7px] uppercase truncate leading-tight">
                            {gcLine1}
                          </div>
                        )}
                        {gcLine2 && (
                          <div className="text-zinc-400 text-[6px] uppercase truncate">
                            {gcLine2}
                          </div>
                        )}
                      </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 rounded-2xl bg-[var(--status-success)] hover:bg-[var(--status-success)]/90 border border-[var(--status-success)]/50 text-caption font-bold uppercase tracking-widest text-white transition-all active:scale-[0.98] shadow-[var(--shadow-md)]"
              >
                GC TAKE
              </button>
              <button
                onClick={handleGcClear}
                className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 rounded-2xl bg-[var(--status-error)] hover:bg-[var(--status-error)]/90 border border-[var(--status-error)]/50 text-caption font-bold uppercase tracking-widest text-white transition-all active:scale-[0.98] shadow-[var(--shadow-md)]"
              >
                GC CLEAR
              </button>
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 rounded-2xl bg-zinc-700 hover:bg-zinc-600 border border-zinc-500/50 text-caption font-bold uppercase tracking-widest text-white transition-all active:scale-[0.98] shadow-[var(--shadow-md)] disabled:opacity-30 disabled:cursor-not-allowed"
                title="Pular crédito do topo da fila"
              >
                PULAR
              </button>
            </div>
            ...
            <div className="border-t border-[var(--border-subtle)] pt-3 flex flex-col gap-2">
              <label className="text-[9px] uppercase tracking-widest text-zinc-600">Presets Rápidos:</label>
              <div className="flex gap-1.5">
                {Object.keys(gcPresets).map((presetName) => (
                  <button
                    key={presetName}
                    onClick={() => handleApplyPreset(presetName)}
                    className="flex-1 px-2 py-1.5 rounded text-[8px] font-bold uppercase tracking-widest bg-zinc-800 hover:bg-zinc-700 border border-white/5 text-zinc-300 transition-all active:scale-95"
                  >
                    {presetName}
                  </button>
                ))}
