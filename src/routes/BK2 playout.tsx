...
export const Route = createFileRoute("/BK2 playout")({
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto p-3 gap-3" style={{ backgroundColor: '#121212' }}>
          ...
          <div className="grid grid-cols-[auto_1fr] gap-3 flex-1">
            {/* ─ Transport Controls ─ */}
            <div className="border rounded-xl p-4 flex flex-col gap-3 w-[220px]" style={{ backgroundColor: '#1a1a1a', borderColor: '#2a2a2a' }}>
              {/* Barra de progresso */}
              <div className="flex items-center gap-2">
              ...
              {/* TRANS slider */}
              <div className="border-t pt-3 flex flex-col gap-1" style={{ borderColor: '#2a2a2a' }}>
                <span className="text-[8px] font-black uppercase tracking-widest text-yellow-500">TRANS</span>
                <div
                  className="relative flex items-center w-full"
                  style={{ height: 48, cursor: "ew-resize" }}
                  onMouseDown={(e) => {
                    handleTransition();
                    const rect = e.currentTarget.getBoundingClientRect();
                ...
                >
                  <div className="absolute inset-x-0 rounded-full" style={{ top: "50%", transform: "translateY(-50%)", height: 6, background: "rgba(234,179,8,0.12)", border: "1px solid rgba(234,179,8,0.2)" }} />
                  <div className="absolute rounded-full transition-none" style={{ left: 0, top: "50%", transform: "translateY(-50%)", height: 6, width: `${transValue}%`, background: "rgba(234,179,8,0.5)" }} />
                  {[0, 25, 50, 75, 100].map((tick) => (
                    <div key={tick} className="absolute" style={{ left: `${tick}%`, top: "50%", transform: "translate(-50%, -50%)", width: tick === 0 || tick === 100 ? 2 : 1, height: tick === 0 || tick === 100 ? 14 : 8, background: "rgba(234,179,8,0.3)", borderRadius: 1 }} />
                  ))}
                  <div className="absolute flex flex-col items-center" style={{ left: `${transValue}%`, top: "50%", transform: "translate(-50%, -50%)", zIndex: 10, pointerEvents: "none" }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: transValue > 0 ? "radial-gradient(circle at 35% 35%, #fde68a, #d97706)" : "radial-gradient(circle at 35% 35%, #6b7280, #374151)", boxShadow: transValue > 0 ? "0 2px 8px rgba(217,119,6,0.7)" : "0 2px 6px rgba(0,0,0,0.5)", border: transValue > 0 ? "1.5px solid rgba(253,230,138,0.5)" : "1.5px solid rgba(107,114,128,0.4)", transition: "background 0.2s, box-shadow 0.2s", marginBottom: -8, overflow: "hidden" }} />
                    <div style={{ width: 22, height: 28, background: "linear-gradient(90deg, #8b7355 0%, #a0826d 40%, #9a7d68 50%, #8b7355 60%, #7a6245 100%)", borderRadius: "6px 6px 0 0", boxShadow: "0 2px 6px rgba(0,0,0,0.7)" }} />
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: transValue > 0 ? "radial-gradient(circle at 35% 35%, #fde68a, #d97706)" : "radial-gradient(circle at 35% 35%, #6b7280, #374151)", boxShadow: transValue > 0 ? "0 2px 8px rgba(217,119,6,0.7)" : "0 2px 6px rgba(0,0,0,0.5)", border: transValue > 0 ? "1.5px solid rgba(253,230,138,0.5)" : "1.5px solid rgba(107,114,128,0.4)", transition: "background 0.2s, box-shadow 0.2s", marginTop: -8, overflow: "hidden" }} />
                  </div>
                </div>
                <div className="flex justify-between">
                  <span className="text-[7px] font-mono font-bold text-zinc-700">PGM</span>
                  <span className="text-[7px] font-mono text-yellow-500" style={{ opacity: transValue > 0 ? 1 : 0.3 }}>{transValue}%</span>
                  <span className="text-[7px] font-mono font-bold text-zinc-700">PVW</span>
                </div>
              </div>
            </div>
